...
internal class WeaviateRetrievableInitializer(private val connection: WeaviateConnection): RetrievableInitializer {
    ...
    override fun initialize() {
        val classExists = connection.client.schema().exists().withClassName(RETRIEVABLE_ENTITY_NAME).run()
        if (classExists.hasErrors()) {
            LOGGER.error { "The $RETRIEVABLE_ENTITY_NAME Collection should be initialized by now." }
            return
        }
    }
    ...
    override fun deinitialize() {
        try {
            connection.client.schema().classDeleter().withClassName(RETRIEVABLE_ENTITY_NAME).run()
        } catch (e: Throwable) {
    ...
    override fun isInitialized(): Boolean =
        connection.client.schema().exists().withClassName(RETRIEVABLE_ENTITY_NAME).run().result
